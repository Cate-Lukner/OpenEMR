...
def find_date(text_file, patient_dict):
    ...
    for date in dates:
        ...
        doc_date = input('No date was found. Could you please type the date of the document? \n')
        doc_date = re.sub(
            '/', '-',
            doc_date 
        )
...
def main():
    ...
    open_pdf = subprocess.Popen("open '%s'" % pdf_name, shell=True, preexec_fn=os.setpgrp)
